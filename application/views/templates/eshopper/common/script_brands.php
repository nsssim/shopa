...
echo '<script>var currency = '.$currency. ';</script>'; 
echo '<script>var rate = '.$rate. ';</script>'; 
...
$(".plus").click(function(){
	//expand its subcategories
	var cat_id = $(this).attr("id").slice(5);
	$("#plus_"+cat_id).hide();
	$("#minus_"+cat_id).show();
	$("#sub_"+cat_id).show("slow");
	//hide other subcategories
	$( ".sub_cat" ).each(function() 
	{
		var current_cat_id = $(this).attr("id").slice(4);
		if(current_cat_id != cat_id)
		{
			$("#plus_"+current_cat_id).show();
			$("#minus_"+current_cat_id).hide();
			$("#sub_"+current_cat_id).hide("slow");	
		}
	});
...
$(".minus").click(function(){
	...
	$("#plus_"+cat_id).show();
	$("#minus_"+cat_id).hide();
	$("#sub_"+cat_id).hide("slow");
});

$(".main_cat").click(function(){
	var cat_id = $(this).attr("id").slice(4);
	
	var v = $("#sub_"+cat_id).is(':visible');
	if(v) // same as minus button
	{
		$("#plus_"+cat_id).show();
		$("#minus_"+cat_id).hide();
		$("#sub_"+cat_id).hide("slow");
	}
	else // same as plus button
	{
		$("#plus_"+cat_id).hide();
		$("#minus_"+cat_id).show();
		$("#sub_"+cat_id).show("slow");
