...
<?php echo '<script>var sa_base_url	 = "'.base_url(). '";</script>'; ?>
...
function gototoletter(divleter){
 ...
 $( document ).ready(function() 
    ...
    return sign + (j ? i.substr(0, j) + thouSeparator : "") + i.substr(j).replace(/(\d{3})(?=\d)/g, "$1" + thouSeparator) + (decPlaces ? decSeparator + Math.abs(n - i).toFixed(decPlaces).slice(2) : "");
		...
		$(".pf").click(function()
		{
			var price_id = $(this).attr('id'); // p7 ,p8 .... 
			var price_id = price_id.slice(1);
			var price_status = $(this).prop('checked'); 
			
			//remove pagination value (example: remove '48?' from http://www.local.com/usa/product/browse/48?cat_id=1601&s=3)
			var current_url = window.location.href;
			const regex = /[0-9]+\?/;
			current_url = current_url.replace(regex, "?");
			//alert(current_url)
			
			//alert(price_id + " : " + price_status + " was ---> " + current_url);	
			// if unchecked 
			if(!price_status)
			{
				var updated_url = current_url.replace("&p="+price_id, "");
				//alert("price_id: " + price_id + " : " + price_status + "\n was ---> " + current_url + "\n now ---> " + updated_url);
			...
			if(price_status)
			{
				//check if qparameter already in url
				var qparameter = 0; // for testing
				if(!qparameter)
				{
					var updated_url = current_url+"&p=" + price_id;
					//alert("price_id: " + price_id + " : " + price_status + "\n was ---> " + current_url + "\n now ---> " + updated_url);
					window.location.href =updated_url;
		...
		//clear all price_filter
		$("#clr_fp").click(function()
		{
			var current_url = window.location.href;
			var ps = Array();
			$(".pf").each(function()
			...
			if(ps.length > 0)
			{
				...
				{
					tmp_str = updated_url.replace("&p="+ps[i], "");
					updated_url = tmp_str;	
					$("#p"+ps[i]).attr('checked', false); 
				}
				//alert(updated_url);
				window.location.href =updated_url;
				
			}
			//alert("was ---> " + current_url + "\n now ---> " + updated_url);
		});
			
		
		//show or hide the clear price button if more than 2 checkboxes are checked
		var c=0;
		$(".pf").each(function()
		{
...
$(document).ready(function(){
	...
	////////////////////script for filter boxes 
	$(".filter_box").click(function()
	{
		
		var current_url = window.location.href ;
		var box_id = $(this).attr("id");
		var filter_str = box_id.replace("_", "="); 
		var new_url = current_url.replace("&"+filter_str, ""); 
		...
		//cat_id=112&mnp=75&mxp=150&s=2&s=4&brnd=30891&brnd=770&clr=15&clr=8&sz=20&sz=16&hl=4&hl=5&s=3
	});
	...
	$("#clear_filters").click(function()
	{
		...
		$(".filter_box").each(function()
		{
			var box_id = $(this).attr("id");
			var filter_str = box_id.replace("_", "="); 
			new_url = new_url.replace("&"+filter_str, ""); 
			//alert(filter_str);
		});
		//alert(new_url);
		//cat_id=112&mnp=75&mxp=150&s=2&s=4&brnd=30891&brnd=770&clr=15&clr=8&sz=20&sz=16&hl=4&hl=5&s=3
...
$("#update_btn").click(function()
	...
	// code for quick brands filter 
		//nassims code https://jsfiddle.net/p9Lo4nwe/6/
		...
		function doneTyping () {
		  //do something
		  var $input_str = $('#bqflrt').val().toLowerCase(); 
		   // alert($input_str);
		   if($input_str.length > 1 )
		   {
		    
		      var $mylist_items = $('.bspan');
		      //show everything if hidden 
		       $mylist_items.each(function()
		      {
		        var $item_iteration_id =  $(this).attr("id");
		        $('#'+$item_iteration_id).show();
		      });
		      
		      $mylist_items.each(function()
		      {
		        var $item_iteration_id =  $(this).attr("id");
		        var $item_iteration_val =  $(this).text().toLowerCase();
		        if ($item_iteration_val.indexOf($input_str) <= 0)
		        {
		           $('#'+$item_iteration_id).hide();
		        }
		  ...
		  else
		  {
		      ...
		      {
		        var $item_iteration_id =  $(this).attr("id");
		        $('#'+$item_iteration_id).fadeIn();
		      });
		  }
		}
		
	//end code for quick brands filter  

//alert(111);
/*$('.pagination_select').keyup(function(event) {
if(event.which==13)
{
  var text = $(this).val();
  alert(text);
...
/*$('.pagination_select').change(function(e)
{
	console.log(e);
...
(function () {
    ...
    $(".pagination_select").on('focus', function () {
