...
<script>
	...
	// ajax call to add new shipping address
	$("#add_btn").bind("click",function()
		{
			$('html, body').css("cursor", "wait");
			
			var target_url = '<?php echo(base_url()."customer/add_new_shipping_address") ; ?>';
			var new_shipping_address_data = $('form#add_new_shipping_address_form').serializeArray();
			...
			(
			{
				url : target_url,
				type: "GET",
				data: new_shipping_address_data,
				cache: false,
				success: function(data_back)
				{
					...
					if(data_back == "1")
					{
						//alert("address was added succesfully");
						//----- CLOSE
						$('.popup').fadeOut(350);
						//----- RELOAD
						location.reload();
				...
				{
					alert("error during adding address");
				}
			});
		});
	
	// end  ajax call to add new address
	
	// ajax call to delete address
	$(".delete_address_btn").bind("click",function()
		{
			$('html, body').css("cursor", "wait");
			
			var target_url = '<?php echo(base_url()."customer/delete_address") ; ?>';
			var oo = 55;
			var address_id = {address_id:$(this).attr('id').slice(3)};
			oo++;
			$.ajax
			(
			{
				...
				success: function(data_back)
				{
					...
					if(data_back == "1")
					{
						alert("address was deleted succesfully");
						//----- RELOAD
						location.reload();
					}
					else
					{
						alert("error during deleting address ");
					}
	...
	// end  ajax call to delete address
	
	//when update address is clicked 
	$(".edit_address_btn").bind("click",function()
	{
		//get the address id from the edit button
		var address_id = $(this).attr("id").slice(3);
		
		//copy  
		var line1_content 		= $("#line_1_"+address_id).val();
		var line2_content 		= $("#line_2_"+address_id).val();
		var line3_content 		= $("#line_3_"+address_id).val();
		var city          		= $("#city_"+address_id).val();
		var country_province 	= $("#country_province_"+address_id).val();
		var zip_code 			= $("#zip_code_"+address_id).val();
		var coutry		 		= $("#coutry_"+address_id).val();
		
		// paste 
		$('#edit_address_id').val(address_id);
		$('#edit_address1').val(line1_content);
		$('#edit_address2').val(line2_content);
		$('#edit_address3').val(line3_content);
		$('#edit_city').val(city);
		$('#edit_region').val(country_province);
		$('#edit_zipcode').val(zip_code);
		$('#edit_country').val(coutry);
	...
	$("#save_btn").bind("click",function()
	{
		...
		var target_url = '<?php echo(base_url()."customer/customer_update_address") ; ?>';
		var new_address_data = $('form#update_shipping_address_form').serializeArray();
		...
		(
		{
			...
			success: function(data_back)
			{
				...
				if(data_back == "1")
				{
					//alert("address was updated succesfully");
					//----- CLOSE
					$('.popup').fadeOut(350);
					//----- RELOAD
					location.reload();
			...
			error: function(jqXHR, textStatus, errorThrown)
			{
				alert("error during updating address");
			}
		});
	});
	  ...
	  .data( "ui-autocomplete" )._renderItem = function( ul, item ) 
	    {
	        ...
	        var temp_toto_itemname=convertToSlug(item.name);
	        var inner_html = '<a href="<?php echo(base_url());?>product/search-' + temp_toto_itemname + '-item-'+ item.id +'.html"><div class="list_autocomplete"><div class="image_autocomplete"><img src="'+ img_url + '"/></div><div class="label_autocomplete">' + item.name + '</div></div></a>';
